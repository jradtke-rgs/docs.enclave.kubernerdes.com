...
dns=1.1.1.1  # temporary until infra-01 is up
```
...
The remaining NVMe space becomes an LVM volume group:
...
lvcreate -L 40G -n lv-infra-01 vg-infra
lvcreate -L 40G -n lv-infra-02 vg-infra
...
mount -o loop /path/to/harvester-v1.3.x-amd64.iso /mnt/harvester-iso
...
LABEL harvester
  ...
  APPEND initrd=harvester/initrd ip=dhcp rd.cos.disable rd.noverifyssl \
    console=tty1 root=live:http://192.168.100.10/harvester/rootfs.squashfs \
    harvester.install.automatic=true \
    harvester.install.config_url=http://192.168.100.10/harvester/config-HOSTNAME.yaml
...
# Copy to nuc-00
ssh-copy-id -i ~/.ssh/id_enclave.pub rke@192.168.100.10
```
...
curl -s -o /dev/null -w "%{http_code}" http://192.168.100.10/harvester/
# Expected: 200
...
# LVM is ready
lvs vg-infra
# Should show lv-infra-01 and lv-infra-02
